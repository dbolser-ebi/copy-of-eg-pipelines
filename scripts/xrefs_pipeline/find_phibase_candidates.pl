...
foreach my $annotation ( @phibase_data ) {
  ...
  if ( !exists($mapped_annotations{$phi}) ) {
    if (scalar(@{$dbas}) <= 10 && scalar(@{$dbas}) > 0) {
       for my $dba (@{$dbas}) {
          ...
          if ( $division eq $opts->{division} && ( $use_all ||  $dba->dbc()->dbname() !~ /collection/) ) {
            ...
            #$taxon_accessions{$tax_id}{$phi}{'ensembl'} = $translation->stable_id;
              ...
              if ($uniprots->{$acc}->{seq} ne '') {
                ...
                my $blast_database = "/nfs/nobackup/ensemblgenomes/hpedro/blast_databases/" . $species . ".pep.all";
                ...
                my $fac = Bio::Tools::Run::StandAloneBlastPlus->new(
                   -db_name => $species,
                   -db_dir => '/nfs/nobackup/ensemblgenomes/hpedro/blast_databases',

...
sub default_options {
  ...
  return {
    ...
    # Add assembly patches, based on transcript evidence from NCBI
    patch_assembly => 1,
    ...
    # We want to align additional sequences from GenBank files with blastn.
    makeblastdb_exe  => 'makeblastdb',
    blast_exe        => 'blastn',
    blast_parameters => '-word_size 11 -num_alignments 100000 -num_descriptions 100000 -lcase_masking ',
...
sub pipeline_wide_parameters {
 ...
 return {
   ...
   'patch_assembly'  => $self->o('patch_assembly'),
...
sub pipeline_analyses {
  ...
  return [
    ...
    {
      -logic_name        => 'PatchAssembly',
      -module            => 'Bio::EnsEMBL::Hive::RunnableDB::Dummy',
      -parameters        => {},
      -rc_name           => 'normal',
      -flow_into         => WHEN('#patch_assembly#' => ['BlastAnalysisSetup']),
    },

    {
      -logic_name      => 'BlastAnalysisSetup',
      -module          => 'Bio::EnsEMBL::EGPipeline::Common::RunnableDB::AnalysisSetup',
      -max_retry_count => 0,
      -batch_size      => 10,
      -parameters      => {
                            logic_name         => 'patch_blastn',
                            display_label      => 'Nucleotide alignments',
                            description        => 'Nucleotide sequences aligned to the genome with <em>blastn</em>.',
                            displayable        => 1,
                            web_data           => undef,
                            db                 => $self->o('fasta_file'),
                            program            => 'blastn',
...

    {
      ...
      -parameters      => {
                            ...
                            db_fasta_file     => $self->o('fasta_file'),
                            database_type     => 'nucl',
                          },
      -rc_name         => 'normal',
      -flow_into       => ['DetectAdditionalSeq'],
    },

    {
      -logic_name      => 'DetectAdditionalSeq',
      -module          => 'Bio::EnsEMBL::EGPipeline::LoadGFF3::DetectAdditionalSeq',
      -max_retry_count => 0,
      -parameters      => {
    ...
    {
      -logic_name      => 'GetSRASeq',
      -module          => 'Bio::EnsEMBL::EGPipeline::LoadGFF3::GetSRASeq',
      -max_retry_count => 0,
      -parameters      => {
                            work_dir => catdir($self->o('pipeline_dir'), 'additional_seq'),
                          },
      -rc_name         => 'normal',
      -flow_into       => {
                            '1->A' => ['LoadAdditionalSeq'],
                            'A->1' => ['Blast'],
                          },
    },

    {
      -logic_name      => 'LoadPatch',
      -module          => 'Bio::EnsEMBL::EGPipeline::LoadGFF3::LoadPatch',
      -max_retry_count => 0,
      -parameters      => {},
      -rc_name         => 'normal',
    },
